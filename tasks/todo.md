...
> 评估日期：2026-06-10 ｜ 基线版本：v0.3.3 ｜ 测试状态：39/39 通过（`python3 -m unittest discover -s tests`）
>
> **2026-06-10 更新：全部 15 项优化已执行完毕**，详见各任务下的 ✅ 备注与 `CHANGELOG.md`。
...
- [x] **P0-1 解决 `dist/openclaw/` 构建产物入库的漂移风险**
  - ✅ 已完成：采用方案 a。`dist/` 移出 git 并加入 `.gitignore`，CI 每次 push 校验 `build_openclaw.py` 可构建。
...
- [x] **P0-2 统一 playbook.md 的双格式冲突**
  - ✅ 已完成：`summarize_lessons`/`render_playbook` 下沉到 `playbook_utils`，`load_playbook_rules` 以 lessons 为准、表格行作为人工补充兜底；`build_playbook.py` 重写为从 lessons 渲染。
...
- [x] **P0-3 修复 `publish_note.py` 发布成功但验证失败时的误报**
  - ✅ 已完成：post 与 verify 拆分；verify 失败记录 `verify_error` 并以 `DONE_WITH_CONCERNS` 退出码 0 返回；action log 记录真实 `markdown_leaks` 检测结果。
...
- [x] **P0-4 统一默认工作区根路径常量**
  - ✅ 已完成：新增 `scripts/workspace_paths.py`，单点定义 PLATFORM/REQUIRED_FILES/vault 根/slugify。
...
- [x] **P1-1 收敛跨脚本重复代码**
  - ✅ 已完成：`parse_metadata`/`read_text`/日历表格解析/`slugify`/标题启发式全部收敛到共享模块（`workspace_parsing`/`workspace_paths`/`title_heuristics`）。
...
- [x] **P1-2 拆分 `collect_xhs_research.py`（786 行）**
  - ✅ 已完成：渲染层拆到 `research_report.py`（collect 从 786 行降到 600 行）；retry/限速保留在采集编排层（测试 mock 依赖该位置，且失败后也统一执行全局限速是刻意设计）。
...
- [x] **P1-3 清理 `diagnose_workspace.py` 的遗留布局兼容逻辑**
  - ✅ 已完成：新增 `scripts/migrate_workspace.py`（默认 dry-run，`--apply` 迁移三种旧布局）；`diagnose_workspace` 只认规范布局并在 stderr 提示旧布局。
...
- [x] **P1-4 补齐测试盲区并加 CI**
  - ✅ 已完成：新增 calendar/learn/score/publish-flow/migration 测试（共 39 个）；GitHub Actions CI 跑 unittest + bundle 构建校验。
...
- [x] **P1-5 健康评分阈值与 references 文档解耦风险**
  - ✅ 已完成：阈值集中到 `assets/diagnosis-thresholds.json`，`score_health` 与 `diagnose_workspace` 共读，rubric 文档加锚定说明，测试断言边界值。
...
- [x] **P2-1 evals.json 缺少执行器**
  - ✅ 已完成：README 标明 evals.json 为外部评测框架的描述性文件，仓库不含 runner。
...
- [x] **P2-2 SKILL.md 中运行时硬编码**
  - ✅ 已完成：SKILL.md description 与 README 改为运行时中立措辞。
...
- [x] **P2-3 `collect_xhs_research.run_and_record` 小瑕疵**
  - ✅ 已完成（结论修正）：复查后确认失败路径的 command-delay 是“每条命令后全局限速”的刻意设计，不是 bug；未做行为改动。
  - 非临时性错误返回前也会执行 `sleep_between_commands`（多睡一次无意义）；循环正常结束的 `unknown_failure` 分支实际不可达，可简化。

- [x] **P2-4 `score_health.py` 缺少时间窗口**
  - ✅ 已完成：`score_health.py` 增加 `--recent`（默认 10，0 为全量），报告标注取样范围。
  - 现状：对 metrics.csv 全量行取均值，老数据会稀释近期表现，与"诊断当前账号状态"的意图不符。
  - 方案：加 `--recent N`（默认取最近 10 条）或按日期窗口过滤，报告中注明取样范围。

- [x] **P2-5 证据脱敏的可追溯性权衡需文档化**
  - ✅ 已完成：`references/research-rubric.md` 新增 Evidence Sanitization Policy 一节。
...
- [x] **P2-6 增加 CHANGELOG**
  - ✅ 已完成：新增 `CHANGELOG.md`（0.3.3 基线 + Unreleased）。
