...
Use this skill as a file-backed operating system for studio and agency delivery. Keep all persistent state in `~/.xiaohongshu/client/<profile>/` and use the bundled scripts to initialize workspaces, generate daily ops, and score account health.
...
3. Append the command result or structured error to `~/.xiaohongshu/client/<profile>/xhs-action-log.md`.
...
python3 scripts/collect_xhs_research.py \
  --brief ~/.xiaohongshu/client/<profile>/01-client-brief.md \
  --output ~/.xiaohongshu/client/<profile>/02-competitor-analysis.md
```
...
```bash
python3 scripts/diagnose_workspace.py --client-dir ~/.xiaohongshu/client/<profile>
```
...
python3 scripts/learn_client_edits.py \
  --client-dir ~/.xiaohongshu/client/<profile> \
...
2. Run `init_client_workspace.py` if `~/.xiaohongshu/client/<profile>/` does not exist.
...
python3 scripts/generate_account_strategy.py \
  --brief ~/.xiaohongshu/client/<profile>/01-client-brief.md \
  --analysis ~/.xiaohongshu/client/<profile>/02-competitor-analysis.md \
  --output ~/.xiaohongshu/client/<profile>/03-account-strategy.md
```
...
python3 scripts/generate_content_calendar.py \
  --brief ~/.xiaohongshu/client/<profile>/01-client-brief.md \
  --strategy ~/.xiaohongshu/client/<profile>/03-account-strategy.md \
  --analysis ~/.xiaohongshu/client/<profile>/02-competitor-analysis.md \
  --output ~/.xiaohongshu/client/<profile>/04-content-calendar.md
```
...
python3 scripts/build_daily_ops.py \
  --brief ~/.xiaohongshu/client/<profile>/01-client-brief.md \
  --calendar ~/.xiaohongshu/client/<profile>/04-content-calendar.md \
  --output ~/.xiaohongshu/client/<profile>/05-daily-ops.md
```
...
python3 scripts/score_health.py \
  --metrics ~/.xiaohongshu/client/<profile>/metrics.csv \
  --output ~/.xiaohongshu/client/<profile>/06-health-report.md
```
